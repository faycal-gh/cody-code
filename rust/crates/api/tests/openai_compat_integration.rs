...
async fn provider_client_dispatches_groq_requests_from_env() {
    let _lock = env_lock();
    let _api_key = ScopedEnvVar::set("GROQ_API_KEY", "gsk-test-key");
    ...
    let server = spawn_server(
        ...
        vec![http_response(
            "200 OK",
            "application/json",
            "{\"id\":\"chatcmpl_provider\",\"model\":\"openai/gpt-oss-120b\",\"choices\":[{\"message\":{\"role\":\"assistant\",\"content\":\"Through Groq provider client\",\"tool_calls\":[]},\"finish_reason\":\"stop\"}],\"usage\":{\"prompt_tokens\":9,\"completion_tokens\":4}}",
        )],
    )
    .await;
    let _base_url = ScopedEnvVar::set("GROQ_BASE_URL", server.base_url());

    let client = ProviderClient::from_model("openai/gpt-oss-120b")
        .expect("Groq provider client should be constructed");
    ...
    assert_eq!(
        request.headers.get("authorization").map(String::as_str),
        Some("Bearer gsk-test-key")
    );
    let body: serde_json::Value = serde_json::from_str(&request.body).expect("json body");
    assert_eq!(body["model"], json!("openai/gpt-oss-120b"));
}
...
async fn provider_client_dispatches_gemini_requests_from_env() {
    ...
    let server = spawn_server(
        ...
        vec![http_response(
            "200 OK",
            "application/json",
            "{\"id\":\"chatcmpl_provider\",\"model\":\"gemini-3-flash-preview\",\"choices\":[{\"message\":{\"role\":\"assistant\",\"content\":\"Through Gemini provider client\",\"tool_calls\":[]},\"finish_reason\":\"stop\"}],\"usage\":{\"prompt_tokens\":9,\"completion_tokens\":4}}",
        )],
    )
    .await;
    let _base_url = ScopedEnvVar::set("GEMINI_BASE_URL", server.base_url());

    let client = ProviderClient::from_model("gemini-3-flash-preview")
        .expect("Gemini provider client should be constructed");
    assert!(matches!(client, ProviderClient::OpenAi(_)));
    ...
    let body: serde_json::Value = serde_json::from_str(&request.body).expect("json body");
    assert_eq!(body["model"], json!("gemini-3-flash-preview"));
}

#[tokio::test]
async fn preserves_gemini_tool_call_extra_content_roundtrip() {
    let state = Arc::new(Mutex::new(Vec::<CapturedRequest>::new()));
    let body = concat!(
        "{",
        ...
        "\"tool_calls\":[{",
        "\"id\":\"call_1\",",
        "\"function\":{\"name\":\"write_file\",\"arguments\":\"{\\\"path\\\":\\\"demo.py\\\"}\"},",
        ...
        messages: vec![
            InputMessage {
                ...
                content: vec![InputContentBlock::ToolUse {
                    ...
                    input: json!({ "path": "existing.py" }),
                    extra_content: Some(json!({
                        "google": {
                            "thought_signature": "sig_existing"
                        }
                    })),
                }],
            },
