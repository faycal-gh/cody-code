...
const HUGGING_FACE_ENV_VARS: &[&str] = &["HF_TOKEN", "HUGGINGFACEHUB_API_TOKEN", "OPENAI_API_KEY"];
...
fn resolve_api_key(config: OpenAiCompatConfig) -> Result<Option<String>, ApiError> {
    if let Some(value) = read_env_non_empty(config.api_key_env)? {
        return Ok(Some(value));
    }

    for key in config.credential_env_vars() {
