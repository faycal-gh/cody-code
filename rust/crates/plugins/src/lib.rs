...
const MANIFEST_RELATIVE_PATH: &str = ".cody-plugin/plugin.json";
...
impl PluginTool {
    ...
    pub fn execute(&self, input: &Value) -> Result<String, PluginError> {
        ...
        process
            ...
            .stdout(Stdio::piped())
            .stderr(Stdio::piped())
            .env("CODY_PLUGIN_ID", &self.plugin_id)
            .env("CODY_PLUGIN_NAME", &self.plugin_name)
            .env("CODY_TOOL_NAME", &self.definition.name)
            .env("CODY_TOOL_INPUT", &input_json);
        if let Some(root) = &self.root {
            process
                .current_dir(root)
                .env("CODY_PLUGIN_ROOT", root.display().to_string());
        }
...
mod tests {
  ...
  "commands": [
    ...
    fn write_lifecycle_plugin(root: &Path, name: &str, version: &str) -> PathBuf {
        ...
        #[cfg(windows)]
        let (init_command, shutdown_command) = (
            (
                "./lifecycle/init.cmd",
        ...
        write_file(
            root.join(init_command.0.trim_start_matches("./")).as_path(),
            init_command.1,
        );
        write_file(
            root.join(shutdown_command.0.trim_start_matches("./"))
                .as_path(),
            shutdown_command.1,
        );
        write_file(
            ...
            format!(
                ...
                init_command.0, shutdown_command.0
            )
            .as_str(),
        );
        log_path
    ...
    fn write_tool_plugin_with_name(root: &Path, name: &str, version: &str, tool_name: &str) {
        #[cfg(windows)]
        let (script_command, script_contents) = (
            "./tools/echo-json.cmd",
            "@echo off\r\necho {\"plugin\":\"%CODY_PLUGIN_ID%\",\"tool\":\"%CODY_TOOL_NAME%\",\"input\":%CODY_TOOL_INPUT%}\r\n",
        );
        #[cfg(not(windows))]
        let (script_command, script_contents) = (
            "./tools/echo-json.sh",
            "#!/bin/sh\nINPUT=$(cat)\nprintf '{\"plugin\":\"%s\",\"tool\":\"%s\",\"input\":%s}\\n' \"$CODY_PLUGIN_ID\" \"$CODY_TOOL_NAME\" \"$INPUT\"\n",
        );
        let script_path = root.join(script_command.trim_start_matches("./"));
        write_file(&script_path, script_contents);
        #[cfg(unix)]
        {
