...
mod tests {
    ...
    fn make_script_executable(_script_path: &Path) {
        #[cfg(unix)]
        {
            let mut permissions = fs::metadata(_script_path).expect("metadata").permissions();
            permissions.set_mode(0o755);
            fs::set_permissions(_script_path, permissions).expect("chmod");
        }
    }
