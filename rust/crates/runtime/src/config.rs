...
pub const CODY_SETTINGS_SCHEMA_NAME: &str = "SettingsSchema";
...
pub struct RuntimeProviderConfig {
    ...
    openai: RuntimeProviderOverride,
    xai: RuntimeProviderOverride,
    groq: RuntimeProviderOverride,
    gemini: RuntimeProviderOverride,
    hugging_face: RuntimeProviderOverride,
}

#[derive(Debug, Clone, PartialEq, Eq, Default)]
pub struct RuntimeProviderOverride {
...
impl ConfigLoader {
    ...
    pub fn discover(&self) -> Vec<ConfigEntry> {
        let user_legacy_path = self.config_home.parent().map_or_else(
            || PathBuf::from(".cody.json"),
            |parent| parent.join(".cody.json"),
        );
        vec![
            ConfigEntry {
            ...
            ConfigEntry {
                ...
                path: self.cwd.join(".cody.json"),
            },
            ConfigEntry {
                source: ConfigSource::Project,
                path: self.cwd.join(".cody").join("settings.json"),
            },
            ConfigEntry {
                source: ConfigSource::Local,
                path: self.cwd.join(".cody").join("settings.local.json"),
            },
        ]
    }
...
impl RuntimeProviderConfig {
    #[must_use]
    pub fn anthropic(&self) -> &RuntimeProviderOverride {
        &self.anthropic
    }

    #[must_use]
    pub fn openai(&self) -> &RuntimeProviderOverride {
        &self.openai
    }

    #[must_use]
    pub fn xai(&self) -> &RuntimeProviderOverride {
        &self.xai
    }

    #[must_use]
    pub fn groq(&self) -> &RuntimeProviderOverride {
        &self.groq
    }

    #[must_use]
    pub fn gemini(&self) -> &RuntimeProviderOverride {
        &self.gemini
    }

    #[must_use]
    pub fn hugging_face(&self) -> &RuntimeProviderOverride {
        &self.hugging_face
    }
}

impl RuntimeProviderOverride {
    #[must_use]
...
pub fn default_config_home() -> PathBuf {
    std::env::var_os("CODY_CONFIG_HOME")
        .map(PathBuf::from)
        .or_else(|| std::env::var_os("HOME").map(|home| PathBuf::from(home).join(".cody")))
        .unwrap_or_else(|| PathBuf::from(".cody"))
}
...
fn read_optional_json_object(
    ...
    let is_legacy_config = path.file_name().and_then(|name| name.to_str()) == Some(".cody.json");
...
fn parse_optional_env_config(root: &JsonValue) -> Result<BTreeMap<String, String>, ConfigError> {
    ...
    optional_string_map(object, "env", "merged settings").map(|value| value.unwrap_or_default())
}
...
fn parse_optional_provider_config(root: &JsonValue) -> Result<RuntimeProviderConfig, ConfigError> {
    ...
    Ok(RuntimeProviderConfig {
        anthropic: parse_provider_override(providers, "anthropic", "merged settings.providers")?,
        openai: parse_provider_override(providers, "openai", "merged settings.providers")?,
        xai: parse_provider_override(providers, "xai", "merged settings.providers")?,
        groq: parse_provider_override(providers, "groq", "merged settings.providers")?,
        gemini: parse_provider_override(providers, "gemini", "merged settings.providers")?,
        hugging_face: parse_provider_override(
            providers,
            "huggingFace",
...
fn parse_provider_override(
    providers: &BTreeMap<String, JsonValue>,
    key: &str,
    context: &str,
) -> Result<RuntimeProviderOverride, ConfigError> {
    let Some(value) = providers.get(key) else {
        return Ok(RuntimeProviderOverride::default());
    };
    let provider = expect_object(value, &format!("{context}.{key}"))?;
    Ok(RuntimeProviderOverride {
        base_url: optional_string(provider, "baseUrl", &format!("{context}.{key}"))?
            .map(str::to_string),
    })
}
...
mod tests {
    ...
    fn rejects_non_object_settings_files() {
        ...
        let home = root.join("home").join(".cody");
    ...
    #[test]
    fn loads_and_merges_cody_code_config_files_by_precedence() {
        let root = temp_dir();
        let cwd = root.join("project");
        let home = root.join("home").join(".cody");
        fs::create_dir_all(cwd.join(".cody")).expect("project config dir");
        fs::create_dir_all(&home).expect("home config dir");

        fs::write(
            home.parent().expect("home parent").join(".cody.json"),
        ...
        assert_eq!(CODY_SETTINGS_SCHEMA_NAME, "SettingsSchema");
        ...
        assert_eq!(loaded.env().get("C").map(String::as_str), Some("3"));
        assert_eq!(
            loaded
                .get("env")
                .and_then(JsonValue::as_object)
        ...
        assert_eq!(
            loaded.providers().groq().base_url(),
            Some("https://api.groq.test/openai/v1")
        );
        assert_eq!(
            loaded.providers().gemini().base_url(),
            Some("https://generativelanguage.googleapis.test/v1beta/openai")
        );
        assert_eq!(
            loaded.providers().hugging_face().base_url(),
            Some("https://router.huggingface.test/v1")
        );
