...
struct ModelPreset {
    label: &'static str,
    model: &'static str,
    provider: ManagedProviderTarget,
}

impl ManagedProviderTarget {
...
const MODEL_PRESETS: &[ModelPreset] = &[
    ModelPreset {
        ...
        provider: ManagedProviderTarget::Anthropic,
    },
    ModelPreset {
    ...
    ModelPreset {
        label: "GPT-5",
        model: "gpt-5",
        provider: ManagedProviderTarget::OpenAi,
    },
    ModelPreset {
    ...
    ModelPreset {
        label: "Grok 3",
        model: "grok-3",
        provider: ManagedProviderTarget::Xai,
    },
    ModelPreset {
        label: "Groq GPT OSS 120B",
        model: "openai/gpt-oss-120b",
        provider: ManagedProviderTarget::Groq,
    },
    ModelPreset {
    ...
    ModelPreset {
        ...
        provider: ManagedProviderTarget::Gemini,
    },
    ModelPreset {
        label: "Qwen 2.5 Coder 32B",
        model: "Qwen/Qwen2.5-Coder-32B-Instruct",
        provider: ManagedProviderTarget::HuggingFace,
    },
    ModelPreset {
        label: "DeepSeek R1",
...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    let defaults = prepare_startup_defaults()?;
    let args: Vec<String> = env::args().skip(1).collect();
    match parse_args_with_defaults(&args, &defaults)? {
        ...
        CliAction::Repl {
            ...
            permission_mode,
            base_url_override,
        } => run_repl(model, allowed_tools, permission_mode, base_url_override)?,
...
fn prepare_startup_defaults() -> Result<CliDefaults, Box<dyn std::error::Error>> {
    ensure_home_dir_on_windows();
    let runtime_config = load_runtime_config_for_current_dir()?;
    apply_config_env_overrides_if_missing(runtime_config.env());
    Ok(CliDefaults {
        model: runtime_config.model().map(ToOwned::to_owned),
...
) -> Result<runtime::RuntimeConfig, Box<dyn std::error::Error>> {
    ensure_home_dir_on_windows();
    let cwd = env::current_dir()?;
    Ok(ConfigLoader::default_for(&cwd).load()?)
}

fn apply_config_env_overrides_if_missing(values: &std::collections::BTreeMap<String, String>) {
...
fn permission_mode_from_resolved(mode: runtime::ResolvedPermissionMode) -> PermissionMode {
    match mode {
        runtime::ResolvedPermissionMode::ReadOnly => PermissionMode::ReadOnly,
        runtime::ResolvedPermissionMode::WorkspaceWrite => PermissionMode::WorkspaceWrite,
        runtime::ResolvedPermissionMode::DangerFullAccess => PermissionMode::DangerFullAccess,
    }
}
...
fn parse_args(args: &[String]) -> Result<CliAction, String> {
    parse_args_with_defaults(args, &CliDefaults::default())
}

#[allow(clippy::too_many_lines)]
fn parse_args_with_defaults(args: &[String], defaults: &CliDefaults) -> Result<CliAction, String> {
    ...
    while index < args.len() {
        match args[index].as_str() {
            ...
            flag if flag.starts_with("--base-url=") => {
                base_url_override = Some(flag[11..].to_string());
                index += 1;
            }
...
fn normalize_allowed_tools(values: &[String]) -> Result<Option<AllowedToolSet>, String> {
    ...
    match current_tool_registry() {
        ...
        Err(_error) => GlobalToolRegistry::builtin().normalize_allowed_tools(values),
    }
}
...
fn provider_display_info(
    model: &str,
    base_url_override: Option<&str>,
) -> Result<ProviderDisplayInfo, Box<dyn std::error::Error>> {
    let runtime_config = load_runtime_config_for_current_dir()?;
    apply_config_env_overrides_if_missing(runtime_config.env());

    let resolved_model = api::resolve_model_alias(model);
    let metadata = api::metadata_for_model(&resolved_model)
        .unwrap_or_else(|| fallback_provider_metadata(api::detect_provider_kind(&resolved_model)));
    let configured_base_url = configured_base_url_for_metadata(&runtime_config, metadata);
    let (effective_base_url, base_url_source) =
        if let Some(base_url) = base_url_override.filter(|value| !value.trim().is_empty()) {
            (base_url.to_string(), "CLI --base-url override".to_string())
        } else if let Some((base_url, source)) = configured_base_url {
            (base_url.to_string(), source.to_string())
        } else if let Some(base_url) = read_non_empty_env(metadata.base_url_env) {
            (
                base_url,
                format!("environment variable {}", metadata.base_url_env),
            )
        } else {
            (
    ...
    Ok(ProviderDisplayInfo {
        provider_kind: metadata.provider,
        provider_label: provider_label_for_metadata(metadata),
        auth_env: metadata.auth_env,
        auth_hint: auth_hint_for_metadata(metadata).to_string(),
...
fn offline_provider_display_info() -> ProviderDisplayInfo {
    ProviderDisplayInfo {
        ...
        base_url_env: "n/a",
        effective_base_url: "not connected".to_string(),
        base_url_source: "resume-only inspection".to_string(),
    }
}
...
fn fallback_provider_metadata(provider_kind: api::ProviderKind) -> api::ProviderMetadata {
    match provider_kind {
        ...
        api::ProviderKind::Xai => api::ProviderMetadata {
            provider: api::ProviderKind::Xai,
            auth_env: "XAI_API_KEY",
            base_url_env: "XAI_BASE_URL",
            default_base_url: "https://api.x.ai/v1",
        },
        api::ProviderKind::OpenAi => api::ProviderMetadata {
...
fn configured_base_url_for_metadata(
    ...
    match (metadata.provider, metadata.auth_env) {
        (api::ProviderKind::ClawApi, _) => runtime_config
            ...
            .anthropic()
            .base_url()
            .map(|url| (url, "settings.providers.anthropic.baseUrl")),
        (api::ProviderKind::Xai, _) => runtime_config
            .providers()
            .xai()
            .base_url()
            .map(|url| (url, "settings.providers.xai.baseUrl")),
        (api::ProviderKind::OpenAi, "GEMINI_API_KEY") => runtime_config
            .providers()
            .gemini()
            .base_url()
            .map(|url| (url, "settings.providers.gemini.baseUrl"))
            .or_else(|| {
                runtime_config
                    .providers()
                    .openai()
                    .base_url()
                    .map(|url| (url, "settings.providers.openai.baseUrl"))
            }),
        (api::ProviderKind::OpenAi, "GROQ_API_KEY") => runtime_config
            .providers()
            .groq()
            .base_url()
            .map(|url| (url, "settings.providers.groq.baseUrl"))
            .or_else(|| {
                runtime_config
                    .providers()
                    .openai()
        ...
        (api::ProviderKind::OpenAi, "HF_TOKEN") => runtime_config
            .providers()
            .hugging_face()
            .base_url()
            .map(|url| (url, "settings.providers.huggingFace.baseUrl"))
            .or_else(|| {
                runtime_config
                    .providers()
                    .openai()
        ...
        (api::ProviderKind::OpenAi, _) => runtime_config
            .providers()
            .openai()
            .base_url()
            .map(|url| (url, "settings.providers.openai.baseUrl")),
...
fn provider_label_for_metadata(metadata: api::ProviderMetadata) -> &'static str {
    match (metadata.provider, metadata.auth_env) {
        ...
        (api::ProviderKind::OpenAi, "HF_TOKEN") => "Hugging Face",
        (api::ProviderKind::OpenAi, _) => "OpenAI-compatible",
    }
}
...
fn auth_hint_for_metadata(metadata: api::ProviderMetadata) -> &'static str {
    match (metadata.provider, metadata.auth_env) {
        ...
        (api::ProviderKind::OpenAi, "HF_TOKEN") => {
            "HF_TOKEN | HUGGINGFACEHUB_API_TOKEN | OPENAI_API_KEY"
        }
        (api::ProviderKind::OpenAi, _) => "OPENAI_API_KEY",
    }
}
...
Usage
  ...
  Manage models and API keys with /models
  Override endpoint with --base-url <url>",
...
fn centered_line(text: &str, width: usize) -> String {
    ...
    format!(
        "{}{}{}",
        " ".repeat(left_pad),
        truncate_display(trimmed, width),
...
fn wrap_display(text: &str, width: usize) -> Vec<String> {
    ...
    for word in text.split_whitespace() {
        ...
        if next_len > width && !current.is_empty() {
            lines.push(current);
            current = truncate_display(word, width);
...
fn panel_row(left: &str, left_width: usize, right: &str, right_width: usize) -> String {
    format!(
        "\x1b[38;5;173m│\x1b[0m {:<left_width$} \x1b[38;5;173m│\x1b[0m {:<right_width$} \x1b[38;5;173m│\x1b[0m",
        truncate_display(left, left_width),
        truncate_display(right, right_width),
        left_width = left_width,
        right_width = right_width,
    )
}

fn startup_recent_activity_lines(current_session_id: &str) -> Vec<String> {
    let Ok(sessions) = list_managed_sessions() else {
        return vec!["Unable to read recent sessions".to_string()];
    };
    ...
    let lines = sessions
        ...
        .take(3)
        .map(|session| format!("{} ({} msgs)", session.id, session.message_count))
        .collect::<Vec<_>>();
...
fn startup_model_summary(model: &str, provider: &ProviderDisplayInfo) -> String {
    format!(
        "{} via {} | {}",
        model, provider.provider_label, provider.auth_hint
    )
}

fn managed_provider_for_model(model: &str) -> ManagedProviderTarget {
    if let Some(metadata) = api::metadata_for_model(model) {
        return match metadata.auth_env {
            "HF_TOKEN" => ManagedProviderTarget::HuggingFace,
            "GEMINI_API_KEY" => ManagedProviderTarget::Gemini,
            "GROQ_API_KEY" => ManagedProviderTarget::Groq,
            "OPENAI_API_KEY" => ManagedProviderTarget::OpenAi,
            "XAI_API_KEY" => ManagedProviderTarget::Xai,
            _ => ManagedProviderTarget::Anthropic,
        };
    }
...
fn managed_provider_from_input(value: &str) -> Option<ManagedProviderTarget> {
    match value.trim().to_ascii_lowercase().as_str() {
        "anthropic" | "claude" => Some(ManagedProviderTarget::Anthropic),
        "openai" | "gpt" | "codex" => Some(ManagedProviderTarget::OpenAi),
        "xai" | "grok" => Some(ManagedProviderTarget::Xai),
        "groq" | "gpt-oss" | "oss" => Some(ManagedProviderTarget::Groq),
        "gemini" | "google" => Some(ManagedProviderTarget::Gemini),
        "huggingface" | "hugging-face" | "hf" | "qwen" | "deepseek" => {
            Some(ManagedProviderTarget::HuggingFace)
        }
        _ => None,
    }
}
...
fn load_local_settings(path: &Path) -> Result<JsonValue, Box<dyn std::error::Error>> {
    match fs::read_to_string(path) {
        ...
        Ok(contents) => {
            ...
            if parsed.is_object() {
                ...
                Err(format!("{} must contain a JSON object", path.display()).into())
            }
        }
...
fn save_local_settings(path: &Path, value: &JsonValue) -> Result<(), Box<dyn std::error::Error>> {
...
fn ensure_json_object<'a>(
    ...
    if !object.get(key).is_some_and(JsonValue::is_object) {
...
fn write_local_model_setting(model: &str) -> Result<PathBuf, Box<dyn std::error::Error>> {
    let path = local_settings_path()?;
    let mut root = load_local_settings(&path)?;
    let object = root
        .as_object_mut()
        .expect("local settings should remain an object");
    object.insert("model".to_string(), JsonValue::String(model.to_string()));
    save_local_settings(&path, &root)?;
    Ok(path)
}

fn write_local_env_setting(
    env_key: &str,
    value: Option<&str>,
    ...
    let object = root
        .as_object_mut()
        .expect("local settings should remain an object");
    let env_object = ensure_json_object(object, "env");
    match value {
        Some(value) => {
            env_object.insert(env_key.to_string(), JsonValue::String(value.to_string()));
        }
        None => {
...
fn write_local_provider_base_url(
    provider: ManagedProviderTarget,
    value: Option<&str>,
) -> Result<PathBuf, Box<dyn std::error::Error>> {
    let path = local_settings_path()?;
    ...
    let object = root
        .as_object_mut()
        .expect("local settings should remain an object");
    let providers = ensure_json_object(object, "providers");
    let provider_object = ensure_json_object(providers, provider.provider_config_key());
    match value {
        Some(value) => {
            provider_object.insert("baseUrl".to_string(), JsonValue::String(value.to_string()));
        }
        None => {
...
fn local_env_value(path: &Path, env_key: &str) -> Option<String> {
    let root = load_local_settings(path).ok()?;
    root.get("env")
...
fn resolve_model_selection(value: &str) -> Result<String, String> {
    ...
    if let Ok(index) = trimmed.parse::<usize>() {
        return MODEL_PRESETS
            ...
            .ok_or_else(|| format!("unknown model preset index: {index}"));
    }
    Ok(resolve_model_alias(trimmed).to_string())
}

fn format_models_manager_report(
    current_model: &str,
    provider: &ProviderDisplayInfo,
    settings_path: &Path,
    ...
    let presets = MODEL_PRESETS
        ...
        .enumerate()
        .map(|(index, preset)| {
            let marker = if preset.model == current_model {
                "● current"
            } else {
                "○ preset"
            };
            format!(
                "  {index:<2} {model:<36} {provider:<11} {marker:<10} {label}",
                index = index + 1,
                model = preset.model,
...
Usage
  ...
  /models key <token>
  /models key <provider> <token>
  /models clear-key [provider]
  /models base-url <url>
  /models base-url clear",
        provider.provider_label,
        provider.auth_env,
        if env::var_os(provider.auth_env).is_some() {
            "yes"
        } else {
            "no"
        },
...
impl LiveCli {
    ...
    fn startup_banner(&self) -> String {
        ...
        let provider = provider_display_info(&self.model, self.base_url_override.as_deref()).ok();
        let provider_label = provider
            .as_ref()
            .map_or("unknown", |info| info.provider_label);
        let base_url = provider.as_ref().map_or_else(
  ...
  Type \x1b[1m/help\x1b[0m for commands | \x1b[2mShift+Enter\x1b[0m for newline | \x1b[2m/exit\x1b[0m to quit",
            self.model,
            provider_label,
            base_url,
...
╚██████╗███████╗██║  ██║╚███╔███╔╝\n\
 ╚═════╝╚══════╝╚═╝  ╚═╝ ╚══╝╚══╝\x1b[0m \x1b[38;5;208mCode\x1b[0m 🦞\n\n\
  ...
  Type \x1b[1m/help\x1b[0m for commands | \x1b[2mShift+Enter\x1b[0m for newline",
    ...
    fn startup_dashboard(&self) -> String {
        let cwd = env::current_dir().map_or_else(
            |_| "<unknown>".to_string(),
            |path| path.display().to_string(),
        );
        let provider = provider_display_info(&self.model, self.base_url_override.as_deref())
            .unwrap_or_else(|_| offline_provider_display_info());
        let left_width = 53;
        let right_width = 62;

        let mut left = vec![
            centered_line("Welcome back!", left_width),
            String::new(),
            centered_line("  .-.   .-.", left_width),
            centered_line(" (  o   o  )", left_width),
            centered_line(" |    ^    |", left_width),
            centered_line(" |  \\___/  |", left_width),
            centered_line("  '-.___.-'", left_width),
            String::new(),
            truncate_display(&startup_model_summary(&self.model, &provider), left_width),
            truncate_display(&format!("cwd: {cwd}"), left_width),
        ];
        ...
        right.push("\x1b[1;38;5;173mRecent activity\x1b[0m".to_string());
        right.extend(startup_recent_activity_lines(&self.session.id));
        right.push(String::new());
        right.extend(wrap_display(
            &format!(
                "Provider: {} | Base URL: {}",
                provider.provider_label, provider.effective_base_url
            ),
            right_width,
        ));
        right.extend(wrap_display(
        ...
        let mut lines = vec![format!(
            ...
            header_label,
            "─".repeat(header_rule.saturating_sub(header_label.chars().count()))
        )];
        lines.push(format!(
        ...
        for (index, (left_line, right_line)) in left.iter().zip(right.iter()).enumerate() {
            lines.push(panel_row(left_line, left_width, right_line, right_width));
            if index == 2 {
                lines.push(format!(
                    "\x1b[38;5;173m├{}┼{}┤\x1b[0m",
                    "─".repeat(left_width + 2),
    ...
    fn run_turn_with_output(
        ...
        match output_format {
            CliOutputFormat::Text => self.run_prompt_text(input),
            CliOutputFormat::Json => self.run_prompt_json(input),
        }
    }

    fn run_prompt_text(&mut self, input: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    fn handle_repl_command(
        ...
        Ok(match command {
            ...
            SlashCommand::Models { args } => {
                self.manage_models(args.as_deref())?;
                true
            }
    ...
    fn refresh_system_prompt(&mut self) -> Result<(), Box<dyn std::error::Error>> {
        self.system_prompt = build_system_prompt()?;
        Ok(())
    }

    fn rebuild_runtime(&mut self, emit_output: bool) -> Result<(), Box<dyn std::error::Error>> {
    ...
    fn manage_models(&mut self, args: Option<&str>) -> Result<(), Box<dyn std::error::Error>> {
        let settings_path = local_settings_path()?;
        let provider = provider_display_info(&self.model, self.base_url_override.as_deref())
            .unwrap_or_else(|_| offline_provider_display_info());
        let current_target = managed_provider_for_model(&self.model);
        let saved_key = local_env_value(&settings_path, provider.auth_env);
        let Some(args) = args.map(str::trim).filter(|value| !value.is_empty()) else {
            println!(
                "{}",
                format_models_manager_report(
                    &self.model,
                    &provider,
                    &settings_path,
                    saved_key.as_deref()
        ...
        match parts.first().copied().unwrap_or_default() {
            ...
            "set" | "use" | "select" => {
                let value = args
                    ...
                    .filter(|value| !value.is_empty())
                    .ok_or("usage: /models set <index|model>")?;
                let model = resolve_model_selection(value)?;
                let previous = self.model.clone();
                let message_count = self.runtime.session().messages.len();
                let path = write_local_model_setting(&model)?;
                self.model = model.clone();
                self.rebuild_runtime(true)?;
                println!(
                    "{}\n  Saved to         {}",
            ...
            "key" | "apikey" | "token" => {
                let (target, token) = match parts.as_slice() {
                    [_, token] => (current_target, (*token).to_string()),
                    [_, provider_name, rest @ ..] if !rest.is_empty() => {
                        let target = managed_provider_from_input(provider_name)
                            .ok_or_else(|| format!("unknown provider `{provider_name}`"))?;
                        (target, rest.join(" "))
                    }
                    _ => {
                        return Err(
                            "usage: /models key <token> or /models key <provider> <token>".into(),
                        )
                    }
                };
                let env_key = target.auth_env();
                let path = write_local_env_setting(env_key, Some(token.trim()))?;
                env::set_var(env_key, token.trim());
                self.rebuild_runtime(true)?;
                println!(
                    "API key updated\n  Provider         {}\n  Env key          {}\n  Saved to         {}\n  Stored value     {}",
                    target.label(),
                    env_key,
                    path.display(),
                    mask_secret(token.trim())
                );
            }
            "clear-key" | "remove-key" => {
                let target = parts
                    .get(1)
                    .and_then(|value| managed_provider_from_input(value))
                    .unwrap_or(current_target);
                let env_key = target.auth_env();
                let path = write_local_env_setting(env_key, None)?;
            ...
            "base-url" => {
                let value = args
                    ...
                    .map(|(_, remainder)| remainder.trim())
                    .filter(|value| !value.is_empty())
                    .ok_or("usage: /models base-url <url>|clear")?;
                let path = if value.eq_ignore_ascii_case("clear") {
                    self.base_url_override = None;
                    write_local_provider_base_url(current_target, None)?
                } else {
                    self.base_url_override = Some(value.to_string());
                    write_local_provider_base_url(current_target, Some(value))?
                };
                self.rebuild_runtime(true)?;
                println!(
                    "Base URL updated\n  Provider         {}\n  Active value     {}\n  Saved to         {}",
                    current_target.label(),
                    self.base_url_override.as_deref().unwrap_or("provider default"),
                    path.display()
                );
            }
            other => {
                return Err(format!(
                    "unknown /models action `{other}`. Use list, set, key, clear-key, or base-url."
                )
                .into());
            }
        }
  ...
  Merged keys       {}",
    ...
    if let Some(section) = section {
        ...
        let value = match section {
            ...
            other => {
                lines.push(format!(
                    "  Unsupported config section '{other}'. Use env, hooks, model, plugins, or providers."
                ));
                return Ok(lines.join(
                    "
",
...
impl DefaultRuntimeClient {
    ...
    fn build_client(
        ...
        let client = match provider.provider_kind {
            ...
            api::ProviderKind::Xai => ProviderClient::Xai(
                api::OpenAiCompatClient::from_env(api::OpenAiCompatConfig::xai())?
                    .with_base_url(provider.effective_base_url.clone()),
            ),
            api::ProviderKind::OpenAi => {
                let config = if provider.auth_env == "HF_TOKEN" {
                    api::OpenAiCompatConfig::hugging_face()
                } else if provider.auth_env == "GEMINI_API_KEY" {
                    api::OpenAiCompatConfig::gemini()
                } else if provider.auth_env == "GROQ_API_KEY" {
                    api::OpenAiCompatConfig::groq()
                } else {
                    api::OpenAiCompatConfig::openai()
                };
                ProviderClient::OpenAi(
                    api::OpenAiCompatClient::from_env(config)?
                        .with_base_url(provider.effective_base_url.clone()),
                )
            }
        };
    ...
    fn provider_client(&mut self) -> Result<ProviderClient, RuntimeError> {
        if self.client.is_none() {
            self.client = Some(
                Self::build_client(&self.model, self.base_url_override.as_deref())
                ...
                .map_err(|error| RuntimeError::new(error.to_string()))?
            {
                match event {
                    ...
                    ApiStreamEvent::ContentBlockStop(_) => {
                        ...
                        if let Some((id, name, input, extra_content)) = pending_tool.take() {
                            if let Some(progress_reporter) = &self.progress_reporter {
    ...
    match block {
        ...
        OutputContentBlock::ToolUse {
            ...
            *pending_tool = Some((
                ...
                name,
                initial_input,
                extra_content.map(|v| v.to_string()),
            ));
        }
...
mod tests {
    use super::{
        ...
        managed_provider_for_model, managed_provider_from_input, normalize_permission_mode,
        parse_args, parse_args_with_defaults, parse_git_status_metadata, permission_policy,
        ...
        status_context, CliAction, CliDefaults, CliOutputFormat, InternalPromptProgressEvent,
        InternalPromptProgressState, ManagedProviderTarget, SlashCommand, StatusUsage,
        DEFAULT_MODEL,
    };
    ...
    #[test]
    fn parses_base_url_flag_for_prompt_mode() {
        let args = vec![
            "--model".to_string(),
            "Qwen/Qwen2.5-Coder-32B-Instruct".to_string(),
            "--base-url=https://router.huggingface.co/v1".to_string(),
            "ship".to_string(),
            "it".to_string(),
        ];
        assert_eq!(
            ...
            CliAction::Prompt {
                ...
                allowed_tools: None,
                permission_mode: PermissionMode::DangerFullAccess,
                base_url_override: Some("https://router.huggingface.co/v1".to_string()),
            }
        );
    }

    #[test]
    fn config_defaults_seed_model_and_permission_mode() {
        let defaults = CliDefaults {
            model: Some("Qwen/Qwen2.5-Coder-32B-Instruct".to_string()),
            permission_mode: Some(PermissionMode::WorkspaceWrite),
        };
        assert_eq!(
            parse_args_with_defaults(&[], &defaults).expect("defaults should parse"),
            CliAction::Repl {
                model: "Qwen/Qwen2.5-Coder-32B-Instruct".to_string(),
                allowed_tools: None,
    ...
    fn repl_help_includes_shared_commands_and_exit() {
        ...
        assert!(help.contains("/models [list|set <model>|key <token>]"));
        assert!(help.contains("/permissions [read-only|workspace-write|danger-full-access]"));
        assert!(help.contains("/clear [--confirm]"));
        assert!(help.contains("/cost"));
        assert!(help.contains("/resume <session-path>"));
        assert!(help.contains("/config [env|hooks|model|plugins|providers]"));
    ...
    fn model_report_uses_sectioned_layout() {
        let report = format_model_report(
            ...
            &super::ProviderDisplayInfo {
                ...
                base_url_env: "ANTHROPIC_BASE_URL",
                effective_base_url: "https://api.anthropic.com".to_string(),
                base_url_source: "provider default".to_string(),
            },
            12,
            4,
        );
        ...
        assert!(report.contains("Provider         Anthropic-compatible"));
        assert!(report.contains("Session messages 12"));
        assert!(report.contains("Switch models with /model <name>"));
        assert!(report.contains("Manage models and API keys with /models"));
    }
    ...
    #[test]
    fn groq_models_map_to_groq_provider_target() {
        assert_eq!(
            managed_provider_for_model("openai/gpt-oss-120b"),
            ManagedProviderTarget::Groq
        );
        assert_eq!(
            managed_provider_for_model("openai/gpt-oss-20b"),
            ManagedProviderTarget::Groq
        );
        assert_eq!(
            managed_provider_from_input("groq"),
            Some(ManagedProviderTarget::Groq)
        );
    }

    #[test]
    fn gemini_models_map_to_gemini_provider_target() {
        assert_eq!(
            managed_provider_for_model("gemini-3-flash-preview"),
            ManagedProviderTarget::Gemini
        );
        assert_eq!(
            managed_provider_from_input("gemini"),
            Some(ManagedProviderTarget::Gemini)
        );
    }
    ...
    fn status_line_reports_model_and_token_totals() {
        ...
        assert!(status.contains("Provider         Anthropic-compatible"));
