...
const SLASH_COMMAND_SPECS: &[SlashCommandSpec] = &[
    ...
    SlashCommandSpec {
        name: "models",
        aliases: &[],
        summary: "Manage models and provider API keys",
        argument_hint: Some("[list|set <model>|key <token>]"),
        resume_supported: false,
    },
    SlashCommandSpec {
        name: "permissions",
    ...
    SlashCommandSpec {
        name: "config",
        aliases: &[],
        summary: "Inspect Cody config files or merged sections",
        argument_hint: Some("[env|hooks|model|plugins|providers]"),
        resume_supported: true,
    },
    SlashCommandSpec {
        name: "memory",
        aliases: &[],
        summary: "Inspect loaded Cody instruction memory files",
        argument_hint: None,
        resume_supported: true,
    },
    SlashCommandSpec {
        name: "init",
        aliases: &[],
        summary: "Create a starter CODY.md for this repo",
        argument_hint: None,
        resume_supported: true,
    },
    SlashCommandSpec {
    ...
    SlashCommandSpec {
        ...
        summary: "Manage Cody Code plugins",
        argument_hint: Some(
...
pub fn handle_commit_push_pr_slash_command(
    ...
    let body_path = write_temp_text_file("cody-pr-body", "md", request.pr_body.trim())?;
...
fn discover_definition_roots(cwd: &Path, leaf: &str) -> Vec<(DefinitionSource, PathBuf)> {
    ...
    for ancestor in cwd.ancestors() {
        ...
        push_unique_root(
            &mut roots,
            DefinitionSource::ProjectCody,
            ancestor.join(".cody").join(leaf),
        );
    }
    ...
    if let Some(home) = env::var_os("HOME") {
        ...
        push_unique_root(
            &mut roots,
            DefinitionSource::UserCody,
            home.join(".cody").join(leaf),
        );
    }
...
fn discover_skill_roots(cwd: &Path) -> Vec<SkillRoot> {
    ...
    for ancestor in cwd.ancestors() {
        ...
        push_unique_skill_root(
            &mut roots,
            DefinitionSource::ProjectCody,
            ancestor.join(".cody").join("skills"),
            SkillOrigin::SkillsDir,
        );
        push_unique_skill_root(
            &mut roots,
        ...
        push_unique_skill_root(
            &mut roots,
            DefinitionSource::ProjectCody,
            ancestor.join(".cody").join("commands"),
            SkillOrigin::LegacyCommandsDir,
        );
    }
    ...
    if let Some(home) = env::var_os("HOME") {
        ...
        push_unique_skill_root(
            &mut roots,
            DefinitionSource::UserCody,
            home.join(".cody").join("skills"),
            SkillOrigin::SkillsDir,
        );
        push_unique_skill_root(
            &mut roots,
            DefinitionSource::UserCody,
            home.join(".cody").join("commands"),
            SkillOrigin::LegacyCommandsDir,
        );
    }
...
fn render_agents_usage(unexpected: Option<&str>) -> String {
    let mut lines = vec![
        ...
        "  Direct CLI       cody agents".to_string(),
        "  Sources          .codex/agents, .cody/agents, $CODEX_HOME/agents".to_string(),
    ];
...
fn render_skills_usage(unexpected: Option<&str>) -> String {
    let mut lines = vec![
        ...
        "  Direct CLI       cody skills".to_string(),
        "  Sources          .codex/skills, .cody/skills, legacy /commands".to_string(),
    ];
    if let Some(args) = unexpected {
        lines.push(format!("  Unexpected       {args}"));
    }
...
mod tests {
    ...
    fn init_git_repo(label: &str) -> PathBuf {
        ...
        run_command(&root, "git", &["config", "user.name", "Cody Tests"]);
        run_command(&root, "git", &["config", "user.email", "cody@example.com"]);
    ...
    fn parses_supported_slash_commands() {
        ...
        assert_eq!(
            SlashCommand::parse("/models set sonnet"),
            Some(SlashCommand::Models {
                args: Some("set sonnet".to_string())
            })
        );
        assert_eq!(
            SlashCommand::parse("/models"),
            Some(SlashCommand::Models { args: None })
        );
        assert_eq!(
    ...
    fn lists_skills_from_project_and_user_roots() {
        ...
        let project_commands = workspace.join(".cody").join("commands");
        ...
        let roots = vec![
            ...
            SkillRoot {
                source: DefinitionSource::ProjectCody,
        ...
        assert!(report.contains("Project (.cody):"));
    ...
    fn agents_and_skills_usage_support_help_and_unexpected_args() {
        ...
        assert!(agents_help.contains("Direct CLI       cody agents"));
    ...
    fn branch_and_worktree_commands_manage_git_state() {
        ...
        let normalized_worktree_path = worktree_path
            .to_str()
            .expect("utf8 path")
            .replace('\\', "/");
        assert!(listed_worktrees.contains(&normalized_worktree_path));
